...
Listagem correx 6. tempo médio de chegada do pedido.
SELECT 
    AVG(DATEDIFF(DAY, dataped, dataentr)) AS TempoMedioChegada
FROM 
    Entrada
	
Listagem correx 7. Total de saída separados por loja.
SELECT l.nome, SUM(s.total) from Saida s, Loja l where l.codloja = s.Loja_codloja GROUP BY l.nome

Listagem correx 8. Listagem dos pesos de entrada e saída.
...
Listagem bernaredo 9. Total de frete gasto na entrada e saída.
