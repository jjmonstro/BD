...
BEGIN
	...
	BEGIN
		...
		BEGIN
		--um while acho que iria tirar 2/3 do código, a idéia é a seguinte, tu sabe que só aquele numero com a seta muda e a (variável tbm com seta). tu pode ficar adicionando 1 nesse número e adicionado o valor na tabela logo depois, com o insert into especificado (não sei se da)

			SELECT @ValorMesAtual = SUM(p.ValorTotal - p.DescAplicado) -- <---- o @Valor mes atual
			FROM Pedidos p
			WHERE p.ClienteID = @CountI
